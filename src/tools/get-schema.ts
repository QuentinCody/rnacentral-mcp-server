...
export function registerGetSchema(server: McpServer, env?: SchemaEnv): void {
    const handler = createGetSchemaHandler("RNACENTRAL_DATA_DO", "rnacentral");

    server.registerTool(
        "rnacentral_get_schema",
        {
            title: "Get Staged Data Schema",
            description:
                "Get schema information for staged RNAcentral data. Shows table structures and row counts. " +
