...
interface QueryEnv {
    RNACENTRAL_DATA_DO?: unknown;
}

export function registerQueryData(server: McpServer, env?: QueryEnv): void {
    const handler = createQueryDataHandler("RNACENTRAL_DATA_DO", "rnacentral");

    server.registerTool(
        "rnacentral_query_data",
        {
            title: "Query Staged RNAcentral Data",
            description:
                "Query staged RNAcentral data using SQL. Use this when responses are too large and have been staged with a data_access_id.",
            inputSchema: {
